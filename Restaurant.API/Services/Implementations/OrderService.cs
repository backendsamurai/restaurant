    ...
    IValidator<CreateOrderModel> validator) : IOrderService
{
    ...
    public async Task<Result<List<OrderResponse>>> GetOrdersByEmployeeAsync(Guid employeeId) =>
        await orderRepository.WhereAsync<OrderResponse>(o => o.Waiter.Id == employeeId);

    public async Task<Result<List<OrderResponse>>> GetOrdersByDeskAsync(Guid deskId) =>
        await orderRepository.WhereAsync<OrderResponse>(o => o.Desk.Id == deskId);
    ...
    public async Task<Result<OrderResponse>> CreateOrderAsync(CreateOrderModel createOrderModel)
    {
        ...
        var waiterRole = await dbContext.EmployeeRoles.FirstOrDefaultAsync(er => er.Name == "waiter");

        if (waiterRole is null)
            return DetailedError.NotFound("Waiter Role not found", "Provide correct waiter role ID");

        var waiter = await dbContext.Employees.FirstOrDefaultAsync(e => e.Id == createOrderModel.WaiterId && e.Role.Id == waiterRole.Id);

        if (waiter is null)
            return DetailedError.NotFound("Waiter not found", "Provide correct employee ID");

        var desk = await dbContext.Desks.FirstOrDefaultAsync(d => d.Id == createOrderModel.DeskId);
        ...
        await dbContext.OrderLineItems.AddRangeAsync(orderItems);
    ...
    {
        var order = await orderRepository.FirstOrDefaultAsync(o => o.Id == orderId);
